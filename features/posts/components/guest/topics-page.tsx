...
export default function TopicsPage({ blogs }: TopicsPageProps) {
  const topics = MAIN_CATEGORIES;

  return (
    <main className="relative min-h-screen bg-gradient-to-b from-transparent to-[#72dbcc]/10 pb-24 font-sans">
      <div
      ...
      <section className="relative mx-auto max-w-7xl px-5 pt-16 sm:px-8 sm:pt-24 lg:pt-20">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className="mt-6 text-lg leading-relaxed text-foreground/80">
            Browse through a curated collection of topics ranging from positive
            psychology and mental health to movie reviews and personal stories.
          </p>
        </div>
      </section>
      ...
      <div className="mx-auto mt-12 flex max-w-5xl flex-wrap justify-center gap-3 px-5 sm:mt-16 sm:px-8">
        {topics.map((topic) => {
          ...
          const hasBlogs = blogs.some((blog) => blog.tags.includes(topic));
      ...
      <div className="mx-auto mt-20 max-w-7xl space-y-24 px-5 sm:px-8 lg:mt-28">
        {topics.map((topic) => {
          const topicBlogs = blogs.filter((blog) => blog.tags.includes(topic));
            ...
            <section key={topic} className="relative scroll-mt-24" id={topic}>
              ...
              >
                ...
                <span className="text-foreground/60">
                  {topicBlogs.length}{" "}
                  {topicBlogs.length === 1 ? "Story" : "Stories"}
                </span>
