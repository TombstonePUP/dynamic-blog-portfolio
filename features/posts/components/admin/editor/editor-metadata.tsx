...
/* ─── Main Metadata Panel ─── */
export default function EditorMetadata({ metadata, onChange, activeSlug }: EditorMetadataProps) {
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto bg-admin-surface">
      ...
      <div className="flex flex-col gap-5 p-5">
        ...
        {/* Tags */}
        <div>
          <FieldLabel icon={Tag} label="Tags" hint="Press Enter to add" />
          <TagInput tags={metadata.tags} onChange={(tags) => update("tags", tags)} />
        </div>
      </div>
    </div>
  );
