...
export default function StaticMdxEditor() {
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col shadow-2xl ring-1 overflow-hidden font-sans select-none">
      <input
        type="file"
        ref={fileInputRef}
        className="hidden"
      ...
      <Modal
        ...
        description="Enter a slug to start a new post."
        icon={<Plus size={20} />}
        footer={
          <>
            <button
              onClick={() => setIsDialogOpen(false)}
              className="px-6 py-2 text-xs font-bold uppercase tracking-widest  hover:text-admin-text"
            >
              Cancel
            </button>
            <button
              onClick={confirmNewPost}
              className="bg-admin-text px-8 py-2.5 text-xs font-bold uppercase tracking-widest text-admin-inverse"
            >
              Go to Story
            </button>
          </>
      ...
      >
        <div className="space-y-4">
          <label className="text-xs font-bold uppercase tracking-widest  px-1">
            Post Slug
          </label>
          <input
            type="text"
            ...
            className="w-full bg-admin-contrast/5 border-none px-4 py-4 text-sm outline-none focus:ring-2 focus:ring-admin-primary/40"
          />
        </div>
      </Modal>
      ...
      <div className="flex items-center justify-between px-6 py-3 border-b  ">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2 px-3 py-1 bg-admin-primary/10 text-[10px] font-black uppercase tracking-widest text-admin-primary ring-1 ring-admin-primary/20">
            Prototype Mode
          </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-2 bg-admin-text px-5 py-2 text-xs font-bold text-admin-inverse transition hover:bg-admin-text/80 disabled:opacity-20 shadow-sm"
          >
            {isSaving ? (
      ...
      <div className="flex flex-1 overflow-hidden relative">
        {/* SIDEBAR */}
        <div
          className=" border-r overflow-hidden flex flex-col shrink-0 shadow-[inset_-10px_0_15px_-15px_rgba(0,0,0,0.1)]"
          style={{ width: showSidebar ? sidebarWidth : 0 }}
        >
          <div
            className="p-4 flex flex-col gap-6 overflow-y-auto flex-1 min-h-0"
          ...
          >
            <div className="flex items-center justify-between text-[11px] font-black uppercase tracking-[0.2em] text-admin-text/60 px-1">
              ...
              <button
                ...
                className="hover:text-admin-primary active:scale-95"
              >
              ...
              {blogFolders.map((folder) => {
                ...
                return (
                  <div key={folder.slug} className="flex flex-col">
                    <div
                      className={`flex items-center group transition ${isActive ? "bg-admin-primary/10 shadow-sm ring-1 ring-admin-primary/5" : "hover:bg-admin-contrast/5"}`}
                    >
                      <button
                        onClick={() => toggleExpand(folder.slug)}
                        className="flex flex-1 items-center gap-3 px-3 py-2.5 text-[13px] font-bold text-left text-admin-text"
                      >
                        {isExpanded ? (
                          <ChevronDown
                            size={14}
                          ...
                          <button
                            ...
                            onClick={() => {
                              if (file.endsWith(".mdx"))
                                handleLoadPost(folder.slug);
                              else handlePreviewImage(folder.slug, file);
                            }}
                            className="flex items-center gap-2.5 px-4 py-2 text-[12px] text-admin-text/60 font-semibold hover:bg-admin-primary/5 hover:text-admin-text"
                          >
                            {file.endsWith(".mdx") ? (
                              <FileCode size={14} />
                            ) : (
        ...
        {showSidebar && (
          <div
            ...
            className="w-1 h-full cursor-col-resize hover:bg-admin-primary transition-colors shrink-0 bg-admin-contrast/5 z-10"
          />
        )}

        {/* INPUT PANE */}
        <div
          className={`flex flex-col transition-all duration-75 overflow-y-auto ${!isSplit ? "items-center" : "w-full"}`}
          style={{ width: isSplit ? editorWidth : "100%" }}
        >
          <textarea
            value={content}
            onChange={(e) => setContent(e.target.value)}
            className={`flex-1 p-10 font-mono text-sm leading-relaxed resize-none focus:outline-none bg-transparent ${!isSplit ? "max-w-4xl" : "w-full"}`}
            spellCheck={false}
          />
        </div>
          ...
          <div className="flex-1 flex flex-col  overflow-y-auto border-l">
            <div className="sticky top-0 z-10 flex items-center justify-between px-8 py-5 /80 backdrop-blur-md border-b ">
              <span className="text-[11px] font-black uppercase tracking-[0.2em] ">
                {previewAsset ? "Media Preview" : "Static Preview"}
              </span>
              {previewAsset && (
                <button
              ...
              {previewAsset ? (
                <div className="flex flex-col items-center gap-8 pt-12">
                  <Image
                    ...
                    height={800}
                    unoptimized
                    className="h-auto max-w-full bg-admin-bg shadow-2xl ring-1"
                  />
                  <div className="p-5 shadow-xl ring-1 text-center">
                    <p className="text-sm font-black text-admin-text">
                ...
                <div className="flex flex-col pt-8">
                  <div className="p-8 shadow-xl ring-1">
                    <h1 className="text-3xl font-black mb-6">Preview Mockup</h1>
                    <div className="space-y-4 text-admin-text/60 leading-relaxed italic">
                      <p>
                        In this prototype version, the MDX is not compiled live
                        to ensure 100% stability in the static export.
                      </p>
                      <p>
                        The layout and styling here exactly match what you would
                        see in the real editor.
                      </p>
                      <hr className="my-8" />
                      <div className="bg-admin-contrast/5 p-6 font-mono text-[11px] overflow-hidden">
                        {content.slice(0, 500)}...
                      </div>
                    </div>
                  </div>
