...
export default function EditorSidebar({
  ...
  return (
    ...
    >
      <div className="flex min-h-0 flex-1 flex-col gap-6 overflow-y-auto p-4" style={{ width }}>
...
function NewDraftPlaceholder() {
  return (
    <div className="mb-4 flex flex-col">
      <div className="rounded bg-admin-primary/20 px-3 py-2.5 text-sm font-black text-admin-text shadow-sm ring-1 ring-admin-primary/30">
      ...
      <div className="mt-1 flex flex-col gap-1 border-l-2 border-admin-primary/20 pl-2">
        <div className="rounded-md bg-admin-primary/10 px-4 py-2 text-[12px] font-black text-admin-text shadow-sm">
        ...
        <div className="mr-2 mt-2 flex flex-col gap-4 rounded-lg bg-admin-primary/5 p-4">
          <div className="flex items-start gap-2.5 text-[11px] font-bold leading-tight text-admin-text">
            <Info size={14} className="mt-0.5 shrink-0 text-admin-primary opacity-60" />
            <span>Set title, excerpt, tags, status, and image URLs directly in the frontmatter.</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="flex flex-col">
      <div
        className={`min-w-0 rounded-lg transition ${
          isActive ? "bg-admin-primary/10 shadow-sm ring-1 ring-admin-primary/5" : "hover:bg-admin-contrast/5"
        }`}
      >
        <button
        ...
        >
          {isExpanded ? (
            <ChevronDown size={14} strokeWidth={3} className="shrink-0 opacity-40" />
          ) : (
      ...
      {isExpanded ? (
        <div className="ml-6 mt-1 flex flex-col gap-0.5 border-l-2 pl-2">
          <button
            ...
            className="flex min-w-0 items-center gap-2.5 rounded-md px-4 py-2 text-left text-[12px] font-semibold text-admin-text/60 transition hover:bg-admin-primary/5 hover:text-admin-text"
          >
            <FileCode size={14} strokeWidth={2.5} className="opacity-60" />
            <span className="truncate">index.mdx</span>
          </button>
          <div className="mt-1 rounded-md px-4 py-2 text-[11px] font-black uppercase tracking-[0.1em] text-admin-text/45">
            {folder.status} · updated {new Date(folder.updatedAt).toLocaleDateString("en-US")}
          </div>
          <div className="px-4 pb-1 text-[11px] text-admin-text/45">{folder.slug}</div>
        </div>
      ) : null}
    </div>
  );
